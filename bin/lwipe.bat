...
for /F "tokens=*" %%i in ('%ADAPTER_BIN% ps -a') do (
	for /F "tokens=*" %%d in ('python %NETKIT_HOME%\python\rm_node_name_from_ps.py %%i') do %ADAPTER_BIN% %%d
)

for /F "tokens=*" %%n in ('%ADAPTER_BIN% network list') do (
	for /F "tokens=*" %%e in ('python %NETKIT_HOME%\python\rm_network_name_from_list.py %%n') do %ADAPTER_BIN% %%e
)
