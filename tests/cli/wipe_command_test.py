...
def test_run_no_params(mock_confirmation_prompt, mock_wipe, mock_get_vlab_temp_path, mock_rm_tree):
    mock_get_vlab_temp_path.return_value = '/vlab/path'
    command = WipeCommand()
    command.run('.', [])
    mock_confirmation_prompt.assert_called_once()
    mock_wipe.assert_called_once_with(all_users=False)
...
@mock.patch("src.Kathara.manager.Kathara.Kathara.wipe")
@mock.patch("src.Kathara.cli.command.WipeCommand.confirmation_prompt")
def test_run_with_force(mock_confirmation_prompt, mock_wipe, mock_get_vlab_temp_path, mock_rm_tree):
    mock_get_vlab_temp_path.return_value = '/vlab/path'
    command = WipeCommand()
    command.run('.', ['-f'])
...
@mock.patch("src.Kathara.cli.command.WipeCommand.confirmation_prompt")
def test_run_with_all(mock_confirmation_prompt, mock_is_admin, mock_wipe, mock_get_vlab_temp_path, mock_rm_tree):
    mock_get_vlab_temp_path.return_value = '/vlab/path'
    mock_is_admin.return_value = True
    command = WipeCommand()
    command.run('.', ['-a'])
    mock_confirmation_prompt.assert_called_once()
    mock_wipe.assert_called_once_with(all_users=True)
    mock_rm_tree.assert_called_once_with('/vlab/path', ignore_errors=True)
